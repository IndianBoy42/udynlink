...
/* Per-module build configuration (generated by mkwasm2c-module into
 * wasm_rt_config.h).  Picked up by every TU that includes this header, so
 * memory-model, page-size and trap-policy defines reach the runtime and the
 * generated code, not just the shim. */
#if defined(__has_include)
#if __has_include("wasm_rt_config.h")
#include "wasm_rt_config.h"
#endif
#endif
...
#define WASM_RT_MAX_CALL_STACK_DEPTH      1000
#endif
#endif
...
/* Call-stack depth counter, incremented/decremented by the FUNC_PROLOGUE /
 * FUNC_EPILOGUE macros in wasm2c-generated code.  Enabling
 * WASM_RT_USE_STACK_DEPTH_COUNT turns wasm recursion into a recoverable
 * WASM_RT_TRAP_EXHAUSTION instead of a native stack overflow. */
#if WASM_RT_USE_STACK_DEPTH_COUNT
extern uint32_t wasm_rt_call_stack_depth;
#endif

/* External linear-memory mode (WASM_RT_EXTERNAL_MEMORY): the host registers
 * a pre-allocated buffer before the module is instantiated, via the
 * generated <module>_set_memory() wrapper.  capacity_bytes is the buffer
 * size; the effective grow limit becomes
 * min(wasm max_pages, capacity_bytes / page_size). */

/* Build-time-selected trap handler (WASM_RT_TRAP_HANDLER, set by
 * mkwasm2c-module --trap-handler).  Declared here, defined by the host:
 * the symbol is resolved from the host at module load time. */
