...
1. **Module Loading** (`udynlink_load_module`):
   ...
   - Allocate or verify RAM region based on load mode (`COPY_ALL`, `COPY_CODE`, `XIP`).
...
2. **Streaming Module Loading** (`udynlink_load_module_stream`):
   ...
   - Supports `COPY_ALL` and `COPY_CODE` only; XIP returns `UDYNLINK_ERR_LOAD_UNABLE_TO_XIP`.
   - For COPY_CODE, the streaming loader internally uses a COPY_ALL-style RAM layout (header+metadata+code+data after LOT) so that `udynlink_lookup_symbol` works correctly after loading.
...
- `udynlink_get_module_name2(const void*)`: Reads module name directly from a base address without loading.
- `udynlink_get_module_size(const void*)`: Computes total size of a module blob from its header.
- `udynlink_get_code_pointer(const udynlink_module_t*)`: Returns pointer to the module's `.text` section in memory.
