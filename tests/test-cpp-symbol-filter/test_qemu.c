#include "udynlink.h"
#include "mod_cpp_filter_module_data.h"
#include "test_utils.h"
#include <stdio.h>

int test_qemu(void) {
    const char *exported_syms[] = {"test", "run_filter_test", NULL};
    const char *extern_syms[] = {"printf", NULL};
    udynlink_module_t mod;
    int res = 0;

    for (int i = (int)UDYNLINK_LOAD_MODE_COPY_ALL; i <= (int)UDYNLINK_LOAD_MODE_XIP; i++) {
        if (udynlink_load_module(&mod, mod_cpp_filter_module_data, NULL, 0, (udynlink_load_mode_t)i))
            return 0;
